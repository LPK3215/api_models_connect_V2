...
def check_import_dependencies():
    ...
    for py_file in src_files:
        ...
        try:
            ...
            # 只保留项目内部导入
            internal_imports = [i for i in imports if i.startswith(("src.", "web.", "tests."))]
    ...
    # 显示主要模块的依赖
    key_modules = ["src/processor.py", "src/cli.py", "src/local/__init__.py"]
...
def run_quick_test():
    ...
    # 测试2: 处理器导入
    try:
        from src.processor import run_pipeline, Processor
    ...
    # 测试3: 云API模块
    try:
        from src.local import get_image_url, process_images_with_cloud_api
    ...
    try:
        ...
        tests.append(("Web应用", True, "create_web_app"))
    except Exception as e:
        tests.append(("Web应用", False, str(e)))
